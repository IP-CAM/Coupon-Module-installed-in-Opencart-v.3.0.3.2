<?php
class ControllerExtensionIngeCouponModuleNews extends Controller {
    ...
    public function index() {
        ...
        $this->load->model('extension/inge_coupon_module/news');

        $this->load->language('extension/inge_coupon_module/news');
...
//        $this->model_extension_module_inge_coupon_module->updateTables();
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_extension_inge_coupon_module_news->addNews($this->request->post);

            $this->session->data['success'] = $this->language->get('text_success');

            $url = $this->getUrl();

            $this->response->redirect($this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news'));
        }
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_extension_inge_coupon_module_news->editNews($this->request->get['news_id'], $this->request->post);
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $news_id) {
                $this->model_extension_inge_coupon_module_news->deleteNews($news_id);
            }
...
//            $url = $this->getUrl();
//
//            $this->response->redirect($this->model_extension_news_opencart_patch_url->link('extension/inge_coupon_module/news'));
//        }
//
//        $this->getList();
//    }
...
//            );
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news')
            );

        $data['add'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/add', $url);
        $data['copy'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/copy', $url);
        $data['delete'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/delete', $url);
        ...
        $results = $this->model_extension_inge_coupon_module_news->getTotalNews($filter_data, $this->config->get('config_language_id'));
            ...
            foreach ($category_total as $category){
                if($category['news_category_id'] == $result['category_id']){
                    $data['all_news'][] = array(
                        ...
                        'edit' => $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/edit','&news_id=' . $result['news_id'] . $url)
                    );
                }
            }
        }
        ...
        $data['news_search'] = $this->model_extension_ws_opencart_patch_url->ajax('extension/inge_coupon_module/news');
        $data['news_autocomplete'] = $this->model_extension_ws_opencart_patch_url->ajax('extension/inge_coupon_module/news/autocomplete');
        ...
        $this->load->model('extension/inge_coupon_module/category');
        $data['sort_title'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news','&sort=pd.title' . $url);
        $data['sort_status'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news','&sort=p.status' . $url);
// $url = $this->getUrl();

        $pagination = new Pagination();
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link('extension/inge_coupon_module/news', $url . '&page={page}');
...
//        $data['results'] = sprintf($this->language->get('text_pagination'), ($product_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($product_total - $this->config->get('config_limit_admin'))) ? $product_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $product_total, ceil($product_total / $this->config->get('config_limit_admin')));
    ...
    protected function getForm() {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news', $url)
            );

        if (!isset($this->request->get['news_id'])) {
            $data['action'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/add', $url);
        } else {
            $data['action'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news/edit', '&news_id=' . $this->request->get['news_id'] . $url);
        }

        $data['cancel'] = $this->model_extension_ws_opencart_patch_url->link('extension/inge_coupon_module/news', $url);

        if (isset($this->request->get['news_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {

            $news_info = $this->model_extension_inge_coupon_module_news->getNews($this->request->get['news_id']);
        ...
        if (isset($this->request->post['news_description'])) {
            ...
            $data['news_description'] = $this->model_extension_inge_coupon_module_news->getNewsDescription($this->request->get['news_id'], $this->config->get('config_language_id'));
        } else {
...
// Categories
        ...
        $categories = $this->model_extension_inge_coupon_module_news->getNewsCategories($this->config->get('config_language_id'));
...
//        var_dump($data['news_categories']);exit;
    ...
    protected function validateForm() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/inge_coupon_module/news')) {
    ...
    protected function getUrl() {
        ...
        if (isset($this->request->get['order']) && $this->request->get['order'] == 'DESC') {
            if($this->request->get['route'] == 'extension/inge_coupon_module/news'){
                $url .= '&order=ASC';
            }else{
                $url .= '&order=DESC';
            }
